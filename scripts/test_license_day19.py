...
def main():
    ...
    c1, b1 = fetch(admin, "/admin/reports")
    ok("admin rapor index", c1 == 200 and "Bayi bazlı" in b1)
    c2, b2 = fetch(admin, "/admin/reports/dealer-count")
    ok("admin bayi sayısı", c2 == 200 and "Lisans sayısı" in b2 and "Toplam" in b2)
    c3, b3 = fetch(admin, "/admin/reports/expiring")
    ok("admin süresi dolan", c3 == 200 and "Süresi dolan" in b3 and "withinDays" in b3)
    c4, b4 = fetch(admin, "/admin/reports/expiring?status=EXPIRED&withinDays=30")
    ok("admin expiring filtre", c4 == 200 and "EXPIRED" in b4 or "Süresi doldu" in b4)

    print("\n[2] Bayi raporları")
    c5, b5 = fetch(dealer, "/dealer/reports")
    ok("dealer rapor index", c5 == 200 and "Lisans özetim" in b5)
    c6, b6 = fetch(dealer, "/dealer/reports/dealer-count")
    ok("dealer özet", c6 == 200 and "Lisans sayısı" in b6)
    c7, b7 = fetch(dealer, "/dealer/reports/expiring?status=ALL&withinDays=30")
    ok("dealer expiring", c7 == 200 and "Yalnızca sizin" in b7)

    print("\n[3] Yetki")
    c8, b8 = fetch(dealer, "/admin/reports/dealer-count")
    ok("dealer admin rapora erişemez", c8 == 403 or "Yetkisiz" in b8 or "403" in b8)
